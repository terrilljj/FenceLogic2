...
const productCategories: ProductCategory[] = [
  {
    ...
    variants: [
      { id: "glass-pool-spigots", name: "Frameless with Spigots" },
...
interface ProductSelectorProps {
  ...
  onSelectVariant: (type: ProductType, variant: ProductVariant) => void;
}

export function ProductSelector({ currentVariant, onSelectVariant }: ProductSelectorProps) {
  return (
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {category.variants.map((variant) => (
                <Badge 
                  key={variant.id}
                  variant={variant.id === currentVariant ? "default" : "outline"}
                  className="cursor-pointer"
                  onClick={() => onSelectVariant(category.id, variant.id)}
