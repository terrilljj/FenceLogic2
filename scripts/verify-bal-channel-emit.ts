import pg from "pg";
import { emitGlassBalChannelSpan } from "../server/services/bom/glass-bal-channel-emit";
...
(async()=>{
  ...
  for(const c of cfgs){
    const span:any={spanId:"A",length:5000,maxPanelWidth:1400,desiredGap:50,channelMounting:"ground",spigotSubstrate:c.sub,fieldValues:{"channel-finish":c.chFin},leftGap:{enabled:true,size:25},rightGap:{enabled:true,size:25}};
    if(c.rail)span.handrail={enabled:true,type:"series-35x35",finish:"satin",startTermination:c.t1,endTermination:c.t2};
    let lay;try{lay=computeSpanLayout({productVariant:"glass-bal-channel",gatesAllowed:false,span});}catch(e:any){um.push(`${c.label}: LAYOUT ${e.message}`);continue;}
    span.panelLayout=lay?.panelLayout;
    const u:string[]=[];for(const l of emitGlassBalChannelSpan({productVariant:"glass-bal-channel"},span,u))skus.add(l.sku);for(const x of u)um.push(`${c.label}: ${x}`);
  }
  ...
  console.log(`BAL-CHANNEL: ${cfgs.length} configs, ${skus.size} SKUs, unmapped=${um.length}, phantom=${phantom.length}, unplaced=${unplaced.length}`);
